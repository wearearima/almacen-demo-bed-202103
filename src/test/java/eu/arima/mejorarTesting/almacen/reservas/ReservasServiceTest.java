...
class ReservasServiceTest {
    ...
    @Nested
    @DisplayName("reservarMedicamento si hay stock suficiente para cubrir las unidades solicitadas")
    class SiHayStockSobrante {
        ...
        void si_stock_reserva_con_unidades_solicitadas() {
            ...
            Reserva resultado = reservasService.reservarMedicamento(ID_FARMACIA, ID_MEDICAMENTO, UNIDADES)
                                               .orElseThrow();

            assertAll("Reserva es correcta",
                    () -> assertEquals(reservasAntes + 1, reservasRepository.count()),
                    () -> assertEquals(UNIDADES, resultado.getUnidades()),
                    () -> assertEquals(ID_MEDICAMENTO, resultado.getIdMedicamento()),
                    () -> assertEquals(ID_FARMACIA, resultado.getIdFarmacia()));
        }
        ...
        void si_stock_actualizar_medicamento() {
            ...
            assertEquals(40, medicamentosRepository.findById(ID_MEDICAMENTO).orElseThrow().getUnidadesStock());
        }
    }

    @Nested
    @DisplayName("reservarMedicamento si no hay stock suficiente para cubrir las unidades solicitadas")
    class SiHayStockPeroMenos {
        ...
        void si_stock_reserva_con_unidades_stock() {
            ...
            assertAll("Reserva es correcta",
                    () -> assertEquals(reservasAntes + 1, reservasRepository.count()),
                    () -> assertEquals(50, resultado.getUnidades()),
                    () -> assertEquals(ID_MEDICAMENTO, resultado.getIdMedicamento()),
                    () -> assertEquals(ID_FARMACIA, resultado.getIdFarmacia()));
        }
        ...
        void si_stock_actualizar_medicamento() {
            ...
            assertEquals(0, medicamentosRepository.findById(ID_MEDICAMENTO).orElseThrow().getUnidadesStock());
        }
    }
